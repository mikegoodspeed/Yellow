...
class CutScene1 (Screen):
    def __init__(self, manager: ScreenManager, font: pygame.font.Font):
        ...
        self.blue_move_duration = 3.0         # move duration for blue
    ...
    def update(self, dt: float):
        ...
        # when fade completes, start blue movement (halfway to right edge)
        if self._blue_fully_visible and not self._blue_finished and not self._blue_moving:
            surf = pygame.display.get_surface()
            if surf:
                w, h = surf.get_size()
                max_center_x = float(w - self.radius)  # rightmost allowed center
            else:
                max_center_x = (self._blue_x or 0)  # fallback
        ...
        # advance blue movement if active
        ...
        # existing red movement logic (no-op if red already finished)
        if self._finished:
            return
